    ...
    $self->{connection}->_push_write(
        Net::AMQP::Frame::Header->new(
            ...
            header_frame => Net::AMQP::Protocol::Basic::ContentHeader->new(
                ...
                content_encoding => $args->{content_encoding} || undef,
                delivery_mode    => $args->{delivery_mode}    || 1,
                priority         => $args->{priority}         || 1,
                correlation_id   => $args->{correlation_id}   || undef,
                expiration       => $args->{expiration}       || undef,
                message_id       => $args->{message_id}       || undef,
                timestamp        => $args->{time}             || time,
                type             => $args->{type}             || undef,
                user_id          => $args->{user_id}          || $self->{connection}->login_user,
                app_id           => $args->{app_id}           || undef,
                cluster_id       => $args->{cluster_id}       || undef,
                %$args,
                headers          => $headers,
            ),
        ),
